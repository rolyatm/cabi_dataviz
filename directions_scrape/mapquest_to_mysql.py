...
URL = 'http://open.mapquestapi.com/directions/v2/route?key=%s&outFormat=json&shapeFormat=raw&generalize=1&routeType=%s&from=%s&to=%s'

#geojson feature string format
newFeature = '{ "type": "Feature", "id": %s, "properties": { "from": %s, "to": %s, "count": %s, "distance":%s, "time":%s}, "geometry": { "type": "LineString", "coordinates": %s } }'
count = 0
narrative = []
...
cursor=cnx.cursor()
addRoute = ("INSERT into routesV2 (s_from, s_to, json, narrative) VALUES (%s, %s, %s, %s)")
...
with open('cabi_trips_2012.csv', 'r') as fr:
        ...
        for r in routes:
                ...
                if (s == d) or (r[0] == 'sTERMINAL_N'):
                        ...
                        count+=1
                #sleep(.25)
                routeData = r[0], r[3], newFeature %(count, r[0], r[3], r[6], distance, time, formattedShape), '<br>'.join(narrative)
